...
// Constructor when using software _spi->  All output pins are configurable.
...
// Constructor when using hardware _spi->  Faster, but must use SPI pins
// specific to each board type (e.g. 11,13 for Uno, 51,52 for Mega, etc.)
Adafruit_ST7735::Adafruit_ST7735(SPIClass *spi, int8_t cs, int8_t dc, int8_t rst) 
  : _spi(spi),Adafruit_GFX(ST7735_TFTWIDTH_128, ST7735_TFTHEIGHT_160) {
  _cs   = cs;
  _dc   = dc;
  _rst  = rst;
  hwSPI = true;
...
inline void Adafruit_ST7735::spiwrite(uint8_t c) {
  ...
  if (hwSPI) {
          ...
          _spi->write(c);// Faster than transfer as we don't need to read   
