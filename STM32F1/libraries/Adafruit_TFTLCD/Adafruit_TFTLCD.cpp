...
void Adafruit_TFTLCD::begin(uint16_t id) {
  ...
  CS_ACTIVE;
  ...
  } else if ((id == 0x9341) || (id == 0x9488) || (id == 0x9486)|| (id == 0x9481)|| (id == 0x9482)) {
	...
	if(id == 0x9488){
		//http://www.51wendang.com/doc/b773d9e460b5009f876ce48e
		writeRegister16(ILI9341_POWERCONTROL1, 0x0F0F);
		writeRegister8(ILI9341_POWERCONTROL2, 0x10);
		writeRegister16(ILI9341_VCOMCONTROL1, 0x1020);
		writeRegister8(ILI9341_MEMCONTROL, ILI9341_MADCTL_MY | ILI9341_MADCTL_BGR);
		writeRegister8(ILI9341_PIXELFORMAT, 0x55);
		writeRegister16(ILI9341_FRAMECONTROL, 0xB011);
		writeRegister8(ILI9341_ENTRYMODE, 0x07);
	}
	else if(id == 0x9486){
	CD_COMMAND;
	write8(ILI9486_CMD_POSITIVE_GAMMA_CTRL);
	CD_DATA;
	write8(0x00);
	write8(0x07);
	write8(0x0f);
	...
	CD_COMMAND;
	write8(ILI9486_CMD_NEGATIVE_GAMMA_CTRL);
	CD_DATA;
	write8(0x00);
	write8(0x22);
	write8(0x24);
	...
	// Power Control 1
	writeRegister16(ILI9486_CMD_POWER_CONTROL_1,0x1010);
	
	// Power Control 2
	writeRegister8(ILI9486_CMD_POWER_CONTROL_2,0x41);	
	
	// VCOM Control
	CD_COMMAND;
	write8(ILI9486_CMD_VCOM_CONTROL_1);
	CD_DATA;
	write8(0x00);
	write8(0x22);
	write8(0x80);
	...
	//writecommand(ILI9486_CMD_MEMORY_ACCESS_CONTROL);
	write8(0x68);
	
	//Interface Mode Control
	// Set up used SPI lines and RGB interface
	writeRegister8(ILI9486_CMD_INTERFACE_MODE_CONTROL,0x00);
	
	//Frame rate 70HZ	
	writeRegister16(ILI9486_CMD_FRAME_RATE_CONTROL_NORMAL,0xB011);
	
	// Display Inversion
	writeRegister8(ILI9486_CMD_DISPLAY_INVERSION_CONTROL,0x02);
	
	// Entry Set Mode
	writeRegister8(ILI9486_CMD_ENTRY_MODE_SET,0xC6);

	// Interface Pixel Format
	writeRegister8(ILI9486_CMD_INTERFACE_PIXEL_FORMAT,0x55);
	// DPI: 16bpp  DBI: 16bpp
	
	// Set Image Function
	// enable 24 bit data bus
	writeRegister8(ILI9486_CMD_SET_IMAGE_FUNCTION,0x01);
	
	// Adjustment Control 3
	// fixed settings and DSI packet setup
	CD_COMMAND;
	write8(ILI9486_CMD_ADJUST_CONTROL_3);
	CD_DATA;
	write8(0xA9);
	write8(0x51);
	write8(0x2C);
	...
	// Set Interface Mode
	CD_COMMAND;
	write8(ILI9486_CMD_DISPLAY_FUNCTION_CONTROL);
	CD_DATA;
	write8(0x00);// RGB or other interface
	write8(0x22);
	write8(0x3B);
	
	writeRegister8(ILI9486_CMD_WRITE_CTRL_DISPLAY,0x2C);
	//Brightness Control ON, Dimming ON,  Blacklight ON
	
	
	//setRotation(1);
	//writeRegister8(ILI9486_INVOFF,0); //Invert Off
	}else if((id==0x9481) || (id==0x9482))
	{
...
void Adafruit_TFTLCD::setAddrWindow(int x1, int y1, int x2, int y2) {
  ...
  } else if ((driver == ID_9341) || (driver == ID_HX8357D) || (driver == ID_9488))  {
    uint32_t t;
...
void Adafruit_TFTLCD::flood(uint16_t color, uint32_t len) {
  ...
  CS_ACTIVE;
  CD_COMMAND;
  if (driver == ID_9341 || (driver == ID_9327) || (driver == ID_9488)) {
    write8(0x2C);
...
void Adafruit_TFTLCD::fillScreen(uint16_t color) {
  ...
  } else if ((driver == ID_9341) || (driver == ID_7575) || (driver == ID_HX8357D) || (driver == ID_9488)) {
...
void Adafruit_TFTLCD::drawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  } else if ((driver == ID_9341) || (driver == ID_HX8357D) || (driver == ID_9327) || (driver == ID_9488)) {
...
void Adafruit_TFTLCD::pushColors(uint16_t *data, uint8_t len, boolean first) {
  ...
  if(first == true) { // Issue GRAM write command only on first call
    ...
    if ((driver == ID_9341) || (driver == ID_HX8357D) || (driver == ID_9327) || (driver == ID_9488)){
       write8(0x2C);
     }  else {
       write8(0x22);
     }
...
uint16_t Adafruit_TFTLCD::readID(void) {
  ...
  */
  ...
  if (id == 0x9341||id == 0x9488||id == 0x9486||id == 0x9481) {
		return id;
  }
